import React, { useEffect } from 'react';

/**
 * مكون إشعارات يظهر في أسفل الشاشة
 * @param {boolean} show - حالة ظهور الإشعار
 * @param {string} message - نص الإشعار
 * @param {string} type - نوع الإشعار ('success', 'error', 'info', 'warning')
 * @param {function} onClose - دالة يتم استدعاؤها عند إغلاق الإشعار
 * @param {number} autoHideDuration - الوقت بالمللي ثانية قبل إخفاء الإشعار تلقائيًا (الافتراضي: 3000 مللي ثانية)
 */
const NotificationToast = ({ 
  show, 
  message, 
  type = 'success', 
  onClose,
  autoHideDuration = 3000
}) => {
  useEffect(() => {
    if (show && onClose) {
      const timer = setTimeout(() => {
        onClose();
      }, autoHideDuration);

      // تنظيف المؤقت عند إلغاء تركيب المكون
      return () => clearTimeout(timer);
    }
  }, [show, onClose, autoHideDuration]);

  if (!show) return null;

  // تعيين الألوان والأيقونة بناءً على نوع الإشعار
  const toastStyles = {
    success: 'text-green-800 bg-green-50 border border-green-200',
    error: 'text-red-800 bg-red-50 border border-red-200',
    info: 'text-blue-800 bg-blue-50 border border-blue-200',
    warning: 'text-yellow-800 bg-yellow-50 border border-yellow-200'
  };

  const getIcon = () => {
    switch(type) {
      case 'success':
        return (
          <svg className="flex-shrink-0 inline w-5 h-5 ml-3" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"></path>
          </svg>
        );
      case 'error':
        return (
          <svg className="flex-shrink-0 inline w-5 h-5 ml-3" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.28 7.22a.75.75 0 00-1.06 1.06L8.94 10l-1.72 1.72a.75.75 0 101.06 1.06L10 11.06l1.72 1.72a.75.75 0 101.06-1.06L11.06 10l1.72-1.72a.75.75 0 00-1.06-1.06L10 8.94 8.28 7.22z" clipRule="evenodd"></path>
          </svg>
        );
      case 'info':
        return (
          <svg className="flex-shrink-0 inline w-5 h-5 ml-3" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd"></path>
          </svg>
        );
      case 'warning':
        return (
          <svg className="flex-shrink-0 inline w-5 h-5 ml-3" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd"></path>
          </svg>
        );
      default:
        return null;
    }
  };

  return (
    <div className={`fixed left-5 bottom-5 z-50 flex items-center p-4 mb-4 text-sm rounded-lg ${toastStyles[type] || toastStyles.info}`}>
      {getIcon()}
      <span className="font-medium">{message}</span>
      {onClose && (
        <button 
          onClick={onClose} 
          className="mr-3 -mt-1 -mb-1 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg p-1.5 inline-flex items-center justify-center h-8 w-8" 
          aria-label="إغلاق"
        >
          <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd"></path>
          </svg>
        </button>
      )}
    </div>
  );
};

export default NotificationToast;